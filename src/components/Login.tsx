...
export default function Login({ onLogin, siteSettings, hasDefaultAdmin = true, intendedDestination, onForgotPassword }: LoginProps) {
  ...
  const {
    siteSettings: contextSiteSettings,
  ...
  useEffect(() => {
    ...
    setSsoProviders(loginSsoProviders(contextSiteSettings || {}));
  }, [contextSiteSettings, settingsLoading]);

  useEffect(() => {
    if (isDemoMode) return undefined;
    const refreshPublicSso = () => {
      void refreshSettings?.();
    };
    const onVis = () => {
      if (document.visibilityState === 'visible') refreshPublicSso();
    };
    window.addEventListener('focus', refreshPublicSso);
    document.addEventListener('visibilitychange', onVis);
    const timer = window.setInterval(refreshPublicSso, 8000);
    return () => {
      window.removeEventListener('focus', refreshPublicSso);
      document.removeEventListener('visibilitychange', onVis);
      window.clearInterval(timer);
    };
  }, [isDemoMode, refreshSettings]);
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-3">
            ...
            {!isDemoMode && (ssoProviders.google || ssoProviders.github || ssoProviders.m365) && (
              <div className="space-y-2">
                ...
                {ssoProviders.m365 && (
                  <SsoLoginButton
                    provider="m365"
                    disabled={isLoading}
                    onClick={() => void handleSsoSignIn('m365')}
                  />
                )}
              </div>
            )}
